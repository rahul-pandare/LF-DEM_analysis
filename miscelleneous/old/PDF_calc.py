        ...
        for k in range(len(ar)):
            ...
            if os.path.exists(dataname):
                for l in range (run[NP[i]]):
                    ...
                    for ii, mat in enumerate(parList[off:]):
                        xp = mat[:,2]
                        zp = mat[:,3]

                        xmat, zmat = np.outer(xp, np.ones(NP[i])), np.outer(zp, np.ones(NP[i]))
                    ...
                    print(f'Done - NP_{str(NP[i])}/phi_{phir}/ar_{str(ar[k])}/run_{l+1}')
