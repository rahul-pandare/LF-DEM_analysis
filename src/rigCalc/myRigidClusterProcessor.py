...
def pebbleGame_LFDEMSnapshot(topDir,parFile,intFile,snapShotRange,returnPebbleIDandObj=True):
    ...
    # Load particle radius (second column), x pos (third column), and z pos(forth column) ** according to latest file format from dev version of LF-DEM
    positionData = np.loadtxt(parFile,usecols=[1,2,3])

    # Extract number of snapshots from positionData
    numSnapshots = int(np.shape(positionData)[0]/numParticles)
    print("numSnapshots = ", numSnapshots)
    ...
    for lines in fileLines:
        # RVP
        numeric_part = lines.split('#')[0].strip()
        arr = np.fromstring(numeric_part, sep=' ')
        if (arr.shape[0] == 0) and ("#" in lines):
    ...
    # The first column is the first particle index, the second is the second particle index and the final column (11th col in latest format) tells us the contact type.
    ...
    print("upperSnapShotRange = ", upperSnapShotRange)
    for i in range(lowerSnapShotRange,upperSnapShotRange):
        if i==numSnapshots-1:
            ...
            currentContacts = np.genfromtxt(itertools.islice(fileLines, int(linesWhereTimestepheaderEnds[i]), int(numLines)),usecols=(0, 1, 10))
            ...
            currentContacts = np.genfromtxt(itertools.islice(fileLines, int(linesWhereTimestepheaderEnds[i]), int(linesWhereDataStarts[i + 1])),usecols=(0, 1, 10))
            currentContacts = currentContacts[np.where(currentContacts[:, 2] > 1), :][0]
            if len(currentContacts)==0:
                contactInfo[counter] = np.array([[0],[0]])
            else:    
...
def rigFileGenerator(topDir,outputDir,snapShotRange,reportIDS=True,stressControlled=True):
    ...
    for currentFile in parFiles:
        ...
        if stressControlled==True:
            result               = re.search('_stress(.*)r_shear', currentFile)
            currentStress        = result.group(1)
            correspondingIntFile = [i for i in intFiles if '_stress'+currentStress+'r_shear' in i]
        else:
        ...
        currentClusterInfo = pebbleGame_LFDEMSnapshot(topDir,currentParFile,currentIntFile,snapShotRange)
...
def rigFileReader(rigFile,snapShotRange,readInIDS=True):
...
def rigidClusterLength(rigFile,parFile,numParticles,Lx,Ly,snapShotRange,rotatePositions=False):
