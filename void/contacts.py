...
topDir      = "/Volumes/Extreme SSD/highBidispersity/"
        ...
        for k in range(len(ar)):
            ...
            if os.path.exists(dataname):
                for l in range (run[NP[ii]]):
                    ...
                    #%% counting small and large particles
                    countSmall = 0
                    countLarge = 0
                    ...
                    totCont = [list(sampleList[:, 10]).count(2) for sampleList in contactList]
                    ...
                    for sampleList in contactList:
                        ...
                        for i in range (sampleList.shape[0]):
                            if int(sampleList[i,10]) == 2 and int(sampleList[i,0]) not in particleIndex:
                                particleIndex.append(sampleList[i,0])
                        TotalParticlesinContact.append(len(particleIndex))
                    ...
                    txtFile.write("# 2 : Total non-sliding frictional contacts on small particles \n")
                    txtFile.write("# 3 : Total number of particles with non-sliding frictional contact on small particles\n\n")
                    
                    txtFile.write("# 4 : Total non-sliding frictional contacts on large particles \n")
                    txtFile.write("# 5 : Total number of particles with non-sliding frictional contact on large particles\n\n")
                    
                    txtFile.write("# 6 : Total non-sliding frictional contacts between small - small particles\n")
                    txtFile.write("# 7 : Total number of particles with non-sliding frictional contact between small - small particles\n\n")
                    
                    txtFile.write("# 8 : Total non-sliding frictional contacts between large - large particles \n")
                    txtFile.write("# 9 : Total number of particles with non-sliding frictional contact between large - large particles \n\n")
                    
                    txtFile.write("# 10 : Total non-sliding frictional contacts between small - large particles \n")
                    txtFile.write("# 11 : Total number of particles with non-sliding frictional contact between small - large particles \n\n")
                        
                    # Write the data from each list side by side
                    for n in range(len(TotalContacts)):
                        row = TotalContacts[n] + totalContactOnSmall[n] + totalContactOnLarge[n] + totalSmallSmall[n] + totalLargeLarge[n] + totalSmallLarge[n]
