...
run         = {500:8,1000:2,2000:2,4000:1}

phi         = [0.70,0.71,0.72,0.73,0.74,0.76,0.77]
            ...
            if os.path.exists(dataname):
                for l in range (run[NP[ii]]):
                    ...
                    for sampleList in contactList:
                        ...
                        particleIndex = []
                        for i in range (sampleList.shape[0]):
                            if (particleSize[int(sampleList[i, 0])] > 1 and particleSize[int(sampleList[i, 1])] < 1) or # type: ignore
                               (particleSize[int(sampleList[i, 0])] < 1 and particleSize[int(sampleList[i, 1])] > 1) and
                               (int(sampleList[i, 10])) == 2: # type: ignore
                               countCont += 2
                               particleIndex.append(sampleList[i, 0])
                               particleIndex.append(sampleList[i, 1])
